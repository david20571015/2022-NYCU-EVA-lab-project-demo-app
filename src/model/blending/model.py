...
from .utils import (reduce_sum, cos_function_weight, gaussian_weight,
                    same_padding, extract_image_patches)
...
def make_layers_transpose(in_channel,
                          out_channel,
                          kernel_size,
                          stride,
                          padding,
...
class identity_block(nn.Module):
    ...
    def __init__(self, channels, norm=True, is_relu=False):
        ...
        self.conv1 = make_layers(channels[0],
                                 channels[1],
                                 kernel_size=1,
                                 stride=1,
                                 padding=0,
                                 ...
                                 activation=True,
                                 is_relu=is_relu)
        self.conv2 = make_layers(channels[1],
                                 channels[2],
                                 kernel_size=3,
                                 stride=1,
                                 padding=1,
                                 ...
                                 activation=True,
                                 is_relu=is_relu)
        self.conv3 = make_layers(channels[2],
                                 channels[3],
                                 kernel_size=1,
                                 stride=1,
                                 padding=0,
...
class convolutional_block(nn.Module):
    ...
    def __init__(self, channels, norm=True, is_relu=False):
        ...
        self.conv3 = make_layers(channels[2],
                                 ...
                                 norm=norm,
                                 activation=False)
        self.shortcut_path = make_layers(channels[0],
                                         channels[3],
                                         kernel_size=1,
                                         stride=2,
                                         padding=0,
...
class SHC(nn.Module):
    ...
    def __init__(self, channel, norm=True):
        ...
        self.conv1 = make_layers(channel * 2,
                                 int(channel / 2),
                                 kernel_size=1,
                                 stride=1,
                                 padding=0,
                                 ...
                                 activation=True,
                                 is_relu=True)
        self.conv2 = make_layers(int(channel / 2),
                                 int(channel / 2),
                                 kernel_size=3,
                                 stride=1,
                                 padding=1,
                                 ...
                                 activation=True,
                                 is_relu=True)
        self.conv3 = make_layers(int(channel / 2),
                                 channel,
                                 kernel_size=1,
                                 stride=1,
                                 padding=0,
...
class LSTM_Encoder_Decoder(nn.Module):
    ...
    def __init__(self, size=[512, 4, 4], split=4, pred_step=4, device=0):
        ...
        #            (the width of the resultant intermediate region will be
        #            the width of input portion * pred_step)

        super(LSTM_Encoder_Decoder, self).__init__()

        self.channel, self.height, self.width = size
        self.lstm_size = self.channel * self.height * int(self.width / split)
        self.LSTM_encoder = nn.LSTM(self.lstm_size,
                                    self.lstm_size,
                                    num_layers=2,
                                    batch_first=True)
        self.LSTM_decoder = nn.LSTM(self.lstm_size,
                                    self.lstm_size,
                                    num_layers=2,
                                    batch_first=True)
        self.output = make_layers(2 * self.channel,
                                  self.channel,
                                  kernel_size=1,
                                  stride=1,
                                  padding=0,
    ...
    def forward(self, x1, x2):
        init_hidden = (Variable(torch.zeros(2, x1.shape[0],
                                            self.lstm_size)).cuda(self.device),
                       Variable(torch.zeros(2, x1.shape[0],
                                            self.lstm_size)).cuda(self.device))
        x1_out = x1
        x2_out = x2
        x1_split = torch.stack(
            torch.split(x1, int(self.width / self.split),
                        dim=3)).view(self.split, -1, 1, self.lstm_size)
        x1_split_reversed = torch.stack(
            torch.split(x1, int(self.width / self.split),
                        dim=3)).flip(dims=[4]).view(self.split, -1, 1,
                                                    self.lstm_size)
        x2_split = torch.stack(
            torch.split(x2, int(self.width / self.split),
                        dim=3)).view(self.split, -1, 1, self.lstm_size)
        x2_split_reversed = torch.stack(
            torch.split(x2, int(self.width / self.split),
                        dim=3)).flip(dims=[4]).view(self.split, -1, 1,
                                                    self.lstm_size)
        ...
        for i in reversed(range(self.split)):
            en_out, en_hidden = self.LSTM_encoder(x1_split_reversed[i],
                                                  en_hidden)
        ...
        de_hidden = hidden_x2
        for i in range(self.split):
            de_out, de_hidden = self.LSTM_decoder(x1_split[i],
                                                  de_hidden)  # f_1^2 ~ f_1^5
        x1_out = torch.cat((x1_out,
                            de_out.view(-1, self.channel, self.height,
                                        int(self.width / self.split))), 3)
        for i in range(self.pred_step + self.split - 1):
            de_out, de_hidden = self.LSTM_decoder(de_out,
                                                  de_hidden)  # f_1^6 ~ f_1^12
            x1_out = torch.cat((x1_out,
                                de_out.view(-1, self.channel, self.height,
                                            int(self.width / self.split))), 3)
        ...
        for i in reversed(range(self.split)):
            de_out, de_hidden = self.LSTM_decoder(x2_split_reversed[i],
                                                  de_hidden)
            # f_2^11' ~ f_2^8'
        x2_out = torch.cat(
            (de_out.view(-1, self.channel, self.height,
                         int(self.width / self.split)).flip(dims=[3]), x2_out),
            3)
        for i in range(self.pred_step + self.split - 1):
            de_out, de_hidden = self.LSTM_decoder(
                de_out, de_hidden)  # f_2^7' ~ # f_2^1'
            x2_out = torch.cat((de_out.view(
                -1, self.channel, self.height, int(
                    self.width / self.split)).flip(dims=[3]), x2_out), 3)

        x1_out = (x1_out[:, :, :, :self.width],
                  x1_out[:, :, :, self.width:-self.width], x1_out[:, :, :,
                                                                  -self.width:])
        x2_out = (x2_out[:, :, :, :self.width],
                  x2_out[:, :, :, self.width:-self.width], x2_out[:, :, :,
                                                                  -self.width:])
    ...
    def forward(self, left, right, mid, shortcut, mask=None):
        ...
        raw_l = [
            extract_image_patches(
                ...
                rates=[1, 1],
                padding='same') for i in range(len(shortcut[0]))
        ]  # [N, C*k*k, L]
        if self.two_input:
            raw_r = [
                extract_image_patches(
                    ...
                    strides=[self.rate * self.stride, self.rate * self.stride],
                    rates=[1, 1],
                    padding='same') for i in range(len(shortcut[1]))
            ]  # [N, C*k*k, L]

        # raw_shape: [N, C, k, k, L]
        raw_l = [
            raw_l[i].view(raw_int_ls[0], raw_int_ls[1], kernel, kernel, -1)
            for i in range(len(raw_l))
        ]
        raw_l = [raw_l[i].permute(0, 4, 1, 2, 3) for i in range(len(raw_l))]
        # raw_shape: [N, L, C, k, k]
        raw_l_groups = [
            torch.split(raw_l[i], 1, dim=0) for i in range(len(raw_l))
        ]
        if self.two_input:
            raw_r = [
                raw_r[i].view(raw_int_rs[0], raw_int_rs[1], kernel, kernel, -1)
                for i in range(len(raw_r))
            ]
            raw_r = [
                raw_r[i].permute(0, 4, 1, 2, 3) for i in range(len(raw_r))
            ]  # raw_shape: [N, L, C, k, k]
            raw_r_groups = [
                torch.split(raw_r[i], 1, dim=0) for i in range(len(raw_r))
            ]

        # downscaling foreground option: downscaling both foreground and
        # background for matching and use original background
        # for reconstruction.
        ...
        mid_groups = torch.split(
            mid, 1, dim=0)  # split tensors along the batch dimension

        # w shape: [N, C*k*k, L]
        left = extract_image_patches(left,
        ...
        if self.two_input:
            right = extract_image_patches(right,
                                          ksizes=[self.ksize, self.ksize],
                                          strides=[self.stride, self.stride],
                                          rates=[1, 1],
                                          padding='same')
        ...
        weight = self.weight_func(raw_int_ls[0],
                                  raw_int_ls[2],
        ...
        for xi, li, ri, batch_idx in zip(mid_groups, l_groups, r_groups, batch):
            ...
            li = li[0]  # [L, C, k, k]
            max_li = torch.sqrt(
                reduce_sum(torch.pow(li, 2) + escape_NaN,
                           axis=[1, 2, 3],
                           keepdim=True))
            li_normed = li / max_li
            if self.two_input:
                ri = ri[0]  # [L, C, k, k]
                max_ri = torch.sqrt(
                    reduce_sum(torch.pow(ri, 2) + escape_NaN,
                               axis=[1, 2, 3],
                               keepdim=True))
                ri_normed = ri / max_ri
            ...
            xi = same_padding(xi, [self.ksize, self.ksize], [1, 1],
                              [1, 1])  # xi: 1*c*H*W
            yi = []
            yi.append(F.conv2d(xi, li_normed,
                               stride=1))  # left   # [1, L, H, W]
            if self.two_input:
                yi.append(F.conv2d(xi, ri_normed,
                                   stride=1))  # right  # [1, L, H, W]
            # conv implementation for fuse scores to encourage large patches
            if self.fuse:
                # make all of depth to spatial resolution
                for i in range(len(yi)):
                    yi[i] = yi[i].view(
                        1, 1, int_ls[2] * int_ls[3], int_mids[2] *
                        int_mids[3])  # (B=1, I=1, H=32*32, W=32*32)
                    yi[i] = same_padding(yi[i], [k, k], [1, 1], [1, 1])
                    yi[i] = F.conv2d(yi[i], fuse_weight,
                                     stride=1)  # (B=1, C=1, H=32*32, W=32*32)
                    yi[i] = yi[i].contiguous().view(
                        1, int_ls[2], int_ls[3], int_mids[2],
                        int_mids[3])  # (B=1, 32, 32, 32, 32)
                    yi[i] = yi[i].permute(0, 2, 1, 4, 3)
                    yi[i] = yi[i].contiguous().view(1, 1, int_ls[2] * int_ls[3],
                                                    int_mids[2] * int_mids[3])
                    yi[i] = same_padding(yi[i], [k, k], [1, 1], [1, 1])
                    yi[i] = F.conv2d(yi[i], fuse_weight, stride=1)
                    yi[i] = yi[i].contiguous().view(1, int_ls[3], int_ls[2],
                                                    int_mids[3], int_mids[2])
                    yi[i] = yi[i].permute(0, 2, 1, 4, 3).contiguous()
            yi = [
                yi[i].view(1, int_mids[2] * int_ls[3], int_mids[2], int_mids[3])
                for i in range(len(yi))
            ...
            for i in range(len(shortcut[0])):
                ...
                # yi = F.pad(yi, [0, 1, 0, 1])
                # # here may need conv_transpose same padding
                y_l[i].append(
                    F.conv_transpose2d(
                        yi[0], li_center, stride=self.rate, padding=1) /
                    4.)  # (B=1, C=128, H=64, W=64)
                if self.two_input:
                    y_r[i].append(
                        F.conv_transpose2d(
                            yi[1], ri_center, stride=self.rate, padding=1) /
                        4.)  # (B=1, C=128, H=64, W=64)

        for i in range(len(shortcut[0])):
            y_l[i] = torch.cat(y_l[i], dim=0).contiguous().view(
                raw_int_ms)  # back to the mini-batch
            if self.two_input:
...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.CA = ContextualAttention(ksize=3,
                                      stride=1,
                                      rate=2,
                                      fuse_k=3,
                                      softmax_scale=10,
        ...
        # Stage 1
        # 256*256*3
        self.encoder_stage1_conv1 = make_layers(3,
                                                64,
                                                kernel_size=4,
                                                stride=2,
                                                padding=1,
        ...
        # 128*128*64
        self.encoder_stage1_conv2 = make_layers(64,
                                                128,
                                                kernel_size=4,
                                                stride=2,
                                                padding=1,
        ...
        # 4*4*1024
        self.feature_in = make_layers(1024,
                                      512,
                                      kernel_size=1,
                                      stride=1,
                                      padding=0,
        ...
        # 4*4*512
        self.feature_out = make_layers(512,
                                       1024,
                                       kernel_size=1,
                                       stride=1,
                                       padding=0,
        ...
        self.skip4 = nn.Sequential(nn.InstanceNorm2d(1024, affine=True),
                                   nn.ReLU())
        ...
        self.skip3 = nn.Sequential(nn.InstanceNorm2d(512, affine=True),
                                   nn.ReLU())
        self.GRB3 = GRB(512, 4)
        ...
        self.skip1 = nn.ReLU()
        self.decoder_stage1 = make_layers_transpose(128,
                                                    64,
                                                    kernel_size=4,
                                                    stride=2,
                                                    padding=1,
    ...
    def decode(self, x, shortcut):
        ...
        if 4 in self.skip:
            ...
            if (4 in self.attention):
                ...
                out[1] = self.skip4(
                    self.SHC4_mid(torch.cat((out[1], sc_m[0]), 1), out[1]))
            out[0] = self.skip4(
                self.SHC4(torch.cat((out[0], shortcut[4][0]), 1),
                          shortcut[4][0]))
            out[2] = self.skip4(
                self.SHC4(torch.cat((out[2], shortcut[4][1]), 1),
                          shortcut[4][1]))
            out = torch.cat((out), 3)
            out = self.GRB4(out)
        ...
        if 3 in self.skip:
            ...
            if (3 in self.attention):
                ...
                sc_m = self.CA(out[0], out[2], out[1], [sc_l, sc_r])
                out[1] = self.skip3(
                    self.SHC3_mid(torch.cat((out[1], sc_m[0]), 1), out[1]))
            out[0] = self.skip3(
                self.SHC3(torch.cat((out[0], shortcut[3][0]), 1),
                          shortcut[3][0]))
            out[2] = self.skip3(
                self.SHC3(torch.cat((out[2], shortcut[3][1]), 1),
                          shortcut[3][1]))
            out = torch.cat((out), 3)
            out = self.GRB3(out)
        ...
        if 2 in self.skip:
            ...
            if (2 in self.attention):
                ...
                sc_m = self.CA(out[0], out[2], out[1], [sc_l, sc_r])
                out[1] = self.skip2(
                    self.SHC2_mid(torch.cat((out[1], sc_m[0]), 1), out[1]))
            out[0] = self.skip2(
                self.SHC2(torch.cat((out[0], shortcut[2][0]), 1),
                          shortcut[2][0]))
            out[2] = self.skip2(
                self.SHC2(torch.cat((out[2], shortcut[2][1]), 1),
                          shortcut[2][1]))
            out = torch.cat((out), 3)
            out = self.GRB2(out)
        ...
        if 1 in self.skip:
            ...
            if (1 in self.attention):
                ...
                sc_m = self.CA(out[0], out[2], out[1], [sc_l, sc_r])
                out[1] = self.skip1(
                    self.SHC1_mid(torch.cat((out[1], sc_m[0]), 1), out[1]))
            out[0] = self.skip1(
                self.SHC1(torch.cat((out[0], shortcut[1][0]), 1),
                          shortcut[1][0]))
            out[2] = self.skip1(
                self.SHC1(torch.cat((out[2], shortcut[1][1]), 1),
                          shortcut[1][1]))
            out = torch.cat((out), 3)
        ...
        if 0 in self.skip:
            ...
            if (0 in self.attention):
                ...
                sc_m = self.CA(out[0], out[2], out[1], [sc_l, sc_r])
                out[1] = self.skip0(
                    self.SHC0_mid(torch.cat((out[1], sc_m[0]), 1), out[1]))
            out[0] = self.skip0(
                self.SHC0(torch.cat((out[0], shortcut[0][0]), 1),
                          shortcut[0][0]))
            out[2] = self.skip0(
                self.SHC0(torch.cat((out[2], shortcut[0][1]), 1),
                          shortcut[0][1]))
            out = torch.cat((out), 3)
